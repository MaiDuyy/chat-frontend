...
const ROLE_META: Record<string, { label: string; color: string; icon: React.ReactNode }> = {
  ...
  MEMBER:  { label: "Thành viên",   color: "text-muted-foreground",  icon: <Users className="w-4 h-4" /> },
  GUEST:   { label: "Khách",        color: "text-muted-foreground",   icon: <UserCheck className="w-4 h-4" /> },
};
...
function DepartmentInviteContent() {
  ...
  if (!userExists) {
    return (
      <FullscreenLayout>
        <Card className="w-full max-w-[480px] shadow-2xl border-red-900/40 bg-slate-900/90 text-foreground backdrop-blur-md overflow-hidden">
          <div className="h-1.5 bg-gradient-to-r from-red-600 to-rose-500 w-full" />
          <CardHeader className="text-center pt-8 pb-4">
            <div className="mx-auto w-16 h-16 bg-red-500/10 rounded-xl border border-red-500/20 flex items-center justify-center mb-4">
          ...
          <CardContent className="pb-2">
            <div className="p-3.5 rounded-lg bg-amber-500/8 border border-amber-500/20 flex gap-3 items-start text-xs text-amber-300 font-medium">
          ...
          <CardFooter className="pb-7 pt-4">
            <Button className="w-full bg-slate-800 hover:bg-slate-750 border border-border text-muted-foreground" onClick={() => router.push("/")}>
              Về trang chủ
            </Button>
          </CardFooter>
        </Card>
        ...
        <Card className="w-full max-w-[480px] shadow-2xl border-border bg-slate-900/90 text-foreground backdrop-blur-md overflow-hidden">
          ...
          <CardHeader className="text-center pt-8 pb-4">
            <div className="mx-auto w-20 h-20 relative mb-5">
              ...
              <div className="absolute inset-0 bg-slate-800 rounded-xl border border-border flex items-center justify-center">
          ...
          <CardContent className="space-y-4 pb-2">
            ...
            <div className="p-4 rounded-lg bg-blue-500/8 border border-blue-500/20 flex gap-3 items-center text-sm text-blue-300">
              <LogIn className="w-5 h-5 shrink-0 text-blue-400" />
              <div>
                <p className="font-semibold text-blue-200 text-xs mb-0.5">Yêu cầu đăng nhập</p>
                <p className="text-xs text-muted-foreground">Vui lòng đăng nhập bằng tài khoản nhận lời mời để tiếp tục.</p>
              </div>
            </div>
          </CardContent>
          <CardFooter className="flex flex-col gap-3 pb-7 pt-4">
            <Button
              className="w-full h-11 rounded-md bg-primary hover:bg-primary/90 text-white font-bold"
            ...
            <Button
              variant="ghost"
              className="w-full h-9 text-xs text-muted-foreground hover:text-muted-foreground"
              onClick={() => router.push("/")}
            >
              Về trang chủ
            </Button>
    ...
    <FullscreenLayout>
      <Card className="w-full max-w-[540px] shadow-2xl border-border bg-slate-900/90 text-foreground backdrop-blur-md overflow-hidden animate-in fade-in zoom-in duration-500">
        ...
        <CardHeader className="text-center pb-3 pt-8">
          <div className="mx-auto w-20 h-20 relative mb-5">
            <div className="absolute inset-0 bg-blue-500/10 rounded-xl rotate-6 animate-pulse" />
            <div className="absolute inset-0 bg-slate-800 rounded-xl border border-border flex items-center justify-center shadow-lg">
              <Building2 className="w-10 h-10 text-blue-400" />
            </div>
          </div>
        ...
        <CardContent className="space-y-4 pt-2 pb-3">
          ...
          {/* Logged-in user info */}
          <div className="flex items-center gap-3 p-3.5 rounded-lg bg-slate-800/60 border border-slate-750">
            <Avatar className="h-9 w-9 rounded-lg">
              <AvatarImage src={(profile as any)?.user?.avatar || (profile as any)?.avatar} />
              <AvatarFallback className="rounded-lg bg-primary/90 text-white text-sm font-bold">
                {(userEmail?.[0] || "U").toUpperCase()}
              </AvatarFallback>
            </Avatar>
            <div className="min-w-0">
              <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">Tài khoản đang đăng nhập</p>
          ...
          {emailMismatch && (
            <div className="p-3.5 rounded-lg bg-red-500/10 border border-red-500/25 flex gap-3 items-start">
          ...
          {/* Ready state */}
          {!emailMismatch && (
            <div className="p-3.5 rounded-lg bg-blue-500/8 border border-blue-500/20 flex items-center gap-3">
        ...
        <CardFooter className="flex gap-3 pb-7 pt-3">
          <Button
            variant="outline"
            className="flex-1 h-11 rounded-md border-border text-muted-foreground hover:text-white hover:bg-slate-800 font-bold"
          ...
          <Button
            className="flex-1 h-11 rounded-md bg-primary hover:bg-primary/90 text-white font-bold transition-all active:scale-[0.98] disabled:opacity-50"
...
function InviteInfoGrid({ inviteData }: { inviteData: any }) {
  ...
  return (
    <div className="grid grid-cols-2 gap-3">
      <div className="p-3.5 rounded-lg bg-slate-800/60 border border-slate-750 flex items-center gap-3">
        <Mail className="w-4 h-4 text-blue-400 shrink-0" />
        <div className="min-w-0">
          <p className="text-[9px] uppercase font-bold text-muted-foreground tracking-wider">Email mời</p>
          <p className="text-xs font-semibold text-muted-foreground truncate">{inviteData?.invitedEmail}</p>
        </div>
      </div>
      <div className="p-3.5 rounded-lg bg-slate-800/60 border border-slate-750 flex items-center gap-3">
        <span className={roleMeta.color}>{roleMeta.icon}</span>
        <div className="min-w-0">
          <p className="text-[9px] uppercase font-bold text-muted-foreground tracking-wider">Vai trò</p>
        ...
        <div className="col-span-2 p-3.5 rounded-lg bg-slate-800/60 border border-slate-750 flex items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            {inviteData.department.description && (
              <p className="text-[10px] text-muted-foreground truncate mt-0.5">{inviteData.department.description}</p>
            )}
          </div>
        </div>
      )}
...
function ErrorCard({ icon, message }: { icon: "link" | "expired"; message: string }) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-slate-950 p-4">
      <Card className="w-full max-w-md shadow-xl border-border bg-slate-900 text-foreground">
        <CardHeader className="text-center pt-8">
          ...
          <CardDescription className="text-muted-foreground text-sm mt-1">{message}</CardDescription>
        </CardHeader>
