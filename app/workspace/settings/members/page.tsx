    ...
    <div className="p-6 space-y-6">
      <div className="flex justify-between items-end gap-4">
        <div className="space-y-1">
          <h1 className="text-lg font-bold font-mono uppercase tracking-tight text-foreground">Thành viên</h1>
          <p className="text-xs font-mono text-muted-foreground dark:text-muted-foreground mt-0.5">Quản lý người dùng và vai trò trong Workspace của bạn.</p>
        </div>
        <Button className="bg-slate-900 hover:bg-slate-800 text-white dark:bg-muted dark:hover:bg-slate-250 dark:text-foreground font-mono font-semibold rounded-sm text-xs h-8 px-3 py-1 flex items-center gap-1.5 shadow-none shrink-0 border border-transparent">
          <UserPlus size={14} />
          Mời thành viên
        </Button>
      </div>
            ...
            {isLoading ? (
              <tr>
                <td colSpan={6} className="py-12 text-center">
                  ...
                  <p className="text-xs text-muted-foreground mt-1.5 font-bold">Đang tải danh sách thành viên...</p>
                </td>
              </tr>
            ) : filteredMembers?.length === 0 ? (
              <tr>
                <td colSpan={6} className="py-12 text-center">
                  <p className="text-xs text-muted-foreground dark:text-muted-foreground">Không tìm thấy thành viên phù hợp</p>
                </td>
              </tr>
            ) : (
              filteredMembers?.map((member) => (
                <tr key={member.id} className="hover:bg-muted/30 dark:hover:bg-white/[0.01] transition-colors duration-150 group">
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2.5">
                      <Avatar className="h-7 w-7 rounded-sm ring-1 ring-slate-100 dark:ring-white/[0.04]">
                        <AvatarImage src={getAvatarUrl(member.user?.avatar, member.user?.name)} className="rounded-sm" />
                        <AvatarFallback className="bg-muted dark:bg-muted text-muted-foreground font-bold text-xs rounded-sm">
                          {member.user?.name?.[0]?.toUpperCase()}
                        </AvatarFallback>
                      </Avatar>
                      <div className="flex items-center gap-1.5">
                        <p className="text-xs font-bold text-foreground truncate max-w-[120px] sm:max-w-none">{member.user?.name}</p>
                        {member.userId === currentUser?.id && (
                          <Badge variant="secondary" className="text-[9px] h-3.5 bg-muted dark:bg-muted text-muted-foreground border border-border rounded-sm px-1 py-0 font-bold">BẠN</Badge>
                        )}
                      </div>
                    </div>
                  </td>
                  <td className="px-4 py-3">
                    <p className="text-muted-foreground dark:text-muted-foreground truncate max-w-[150px] sm:max-w-none">{member.user?.email}</p>
                  </td>
                  <td className="px-4 py-3">
                    <Badge
                      className={`
                        text-[9px] font-bold px-1.5 py-0.5 rounded-sm border shadow-none font-mono
                        ${member.role === 'WORKSPACE_OWNER' ? 'text-amber-700 border-amber-250/60 bg-amber-50/50 dark:text-amber-400 dark:border-amber-900/30 dark:bg-amber-950/20' :
                          member.role === 'WORKSPACE_ADMIN' ? 'text-blue-700 border-blue-250/60 bg-blue-50/50 dark:text-blue-400 dark:border-blue-900/30 dark:bg-blue-950/20' :
                            'text-muted-foreground border-slate-250/60 bg-muted/50 dark:text-muted-foreground dark:border-white/[0.06] dark:bg-muted/50'}
                      `}
                    >
                      {member.role}
                    </Badge>
                  ...
                  <td className="px-4 py-3">
                    <p className="text-muted-foreground dark:text-zinc-550">
                      {format(new Date(member.joinedAt || Date.now()), 'dd/MM/yyyy', { locale: vi })}
                    </p>
                  </td>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-1.5">
                      <div className={`h-1.5 w-1.5 rounded-full ${onlineUsers.has(member.userId) || member.userId === currentUser?.id ? 'bg-emerald-500 animate-pulse' : 'bg-muted dark:bg-zinc-700'}`} />
                      <span className="text-[11px] font-semibold text-muted-foreground dark:text-slate-450">
                    ...
                    {isAdmin && member.userId !== currentUser?.id && member.role !== 'OWNER' && (
                      <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                          <Button variant="ghost" size="icon" className="h-6 w-6 rounded-sm hover:bg-muted dark:hover:bg-muted hover:shadow-none transition-colors border border-transparent hover:border-slate-250/60 dark:hover:border-white/[0.08]">
                        ...
                        <DropdownMenuContent align="end" className="w-48 p-1 rounded-sm border-border bg-background font-mono text-xs">
                          <div className="px-2 py-1 text-[9px] font-bold text-muted-foreground uppercase tracking-wider">Thay đổi vai trò</div>
                          <DropdownMenuItem onClick={() => handleRoleChange(member.userId, 'WORKSPACE_ADMIN')} className="rounded-sm py-1 cursor-pointer text-xs">
                            <Shield className="w-3.5 h-3.5 mr-2 text-primary" />
                            <span className="font-semibold text-muted-foreground dark:text-muted-foreground">Admin</span>
                            {member.role === 'WORKSPACE_ADMIN' && <Check className="w-3.5 h-3.5 ml-auto text-blue-600" />}
                          </DropdownMenuItem>
                          <DropdownMenuItem onClick={() => handleRoleChange(member.userId, 'WORKSPACE_MEMBER')} className="rounded-sm py-1 cursor-pointer text-xs">
                            <Users className="w-3.5 h-3.5 mr-2 text-muted-foreground dark:text-muted-foreground" />
                            <span className="font-semibold text-muted-foreground dark:text-muted-foreground">Member</span>
                          ...
                          {isOwner && (
                            <>
                              ...
                              <DropdownMenuItem
                                ...
                                className="rounded-sm py-1 cursor-pointer text-amber-700 focus:text-amber-800 dark:text-amber-400 focus:bg-amber-50/50 dark:focus:bg-amber-950/20 text-xs"
                              >
                          ...
                          <DropdownMenuItem
                            ...
                            className="text-red-650 dark:text-red-400 focus:bg-red-50/50 dark:focus:bg-red-950/20 rounded-sm py-1 cursor-pointer text-xs"
                          >
