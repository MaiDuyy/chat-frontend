...
export function AuditLogTable() {
    ...
    return (
        <div className="space-y-3">
            ...
            <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                <div className="flex flex-wrap items-center gap-2">
                    ...
                    <Button
                        ...
                        className={cn("h-8 text-xs rounded-md px-2.5 border-border bg-secondary/30", showFilters && "bg-secondary text-primary")}
                    >
                        <Filter className="w-3.5 h-3.5 mr-1.5" />
                        Bộ lọc
                    </Button>
                </div>
                <Button variant="outline" size="sm" className="h-8 text-xs rounded-md px-2.5 border-border bg-secondary/30">
            ...
            {/* Expanded Filters */}
            {showFilters && (
                <div className="flex flex-wrap gap-3 p-3 bg-muted dark:bg-slate-900/30 rounded-md border border-border/50">
                    ...
                    <TableBody>
                        {filteredLogs.length === 0 ? (
                            ...
                            filteredLogs.map((log: AuditLog) => (
                                <TableRow key={log.id} className="hover:bg-muted/40 dark:hover:bg-slate-800/40 border-b border-border/50 last:border-0">
                                    ...
                                    <TableCell className="py-2 px-3">
                                        <span className="text-xs font-medium text-foreground dark:text-foreground">{log.resource}</span>
                                        {log.resourceId && (
                                    ...
                                    <TableCell className="py-2 px-3 text-right">
                                        <Button
                                            variant="ghost"
                                            size="icon"
                                            className="h-7 w-7 rounded-md hover:bg-muted dark:hover:bg-slate-800"
            ...
            <Dialog open={!!selectedLog} onOpenChange={() => setSelectedLog(null)}>
                <DialogContent className="rounded-lg max-w-xl border-border bg-card text-xs">
                    <DialogHeader>
                        <DialogTitle className="text-sm font-semibold text-foreground border-b border-border pb-2">
                        ...
                        <div className="space-y-3 pt-1">
                            <div className="grid grid-cols-2 gap-3">
                                ...
                                {selectedLog.ipAddress && (
                                    <div>
                                        ...
                                        <p className="font-mono text-foreground dark:text-slate-350 font-semibold mt-0.5">{selectedLog.ipAddress}</p>
                                    </div>
                                )}
                            </div>
                            ...
                            {selectedLog.details && (
                                <div className="space-y-1">
                                    ...
                                    <pre className="p-2.5 bg-slate-950 dark:bg-slate-900 text-foreground rounded-md text-[10px] font-mono overflow-auto max-h-40 border border-border">
