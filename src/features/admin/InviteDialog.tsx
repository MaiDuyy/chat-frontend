...
export function InviteDialog({ open, onOpenChange }: InviteDialogProps) {
    ...
    return (
        <Dialog open={open} onOpenChange={handleClose}>
            <DialogContent className="sm:max-w-[520px] p-0 overflow-hidden border border-border dark:border-border rounded-lg shadow-2xl bg-white dark:bg-slate-950">
                {/* Visual Header */}
                <DialogHeader className="px-6 pt-5 pb-4 border-b border-border dark:border-slate-850 bg-muted/60 dark:bg-slate-900/40">
                    <DialogTitle className="flex items-center gap-2.5 text-base font-bold text-foreground dark:text-slate-150">
                        <UserPlus className="w-5 h-5 text-blue-500" />
                        Quản lý Thêm Nhân sự Mới
                    </DialogTitle>
                    <DialogDescription className="text-muted-foreground dark:text-muted-foreground text-xs mt-1">
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-2 p-1 bg-muted dark:bg-slate-900 rounded-md border border-border/40 dark:border-border/40">
                        <button
                            type="button"
                            onClick={() => {
                                setFlowMode('C');
                            ...
                            className={`flex items-center justify-center gap-1.5 py-2 text-xs font-semibold rounded-md transition-all cursor-pointer ${
                                flowMode === 'C'
                                    ? 'bg-white dark:bg-slate-800 text-primary shadow-sm border border-border/50 dark:border-border/50'
                                    : 'text-muted-foreground hover:text-foreground dark:hover:text-slate-350'
                            }`}
                        >
                        ...
                        <button
                            ...
                            className={`flex items-center justify-center gap-1.5 py-2 text-xs font-semibold rounded-md transition-all cursor-pointer ${
                                flowMode === 'A'
                                    ? 'bg-white dark:bg-slate-800 text-amber-600 dark:text-amber-400 shadow-sm border border-border/50 dark:border-border/50'
                                    : 'text-muted-foreground hover:text-foreground dark:hover:text-slate-350'
                            }`}
                        >
                    ...
                    <div className={`p-3 rounded-md border text-[11px] leading-relaxed font-medium transition-all ${
                        flowMode === 'A'
                    ...
                    <div className="space-y-1.5">
                        <Label htmlFor="invite-email" className="text-xs font-semibold text-muted-foreground dark:text-muted-foreground">Địa chỉ Email *</Label>
                        <div className="relative">
                            ...
                            <Input
                                ...
                                className="pl-9 h-9 text-xs rounded-md border-border dark:border-slate-750 bg-white dark:bg-slate-900 focus-visible:ring-1 focus-visible:ring-blue-550"
                            />
                        </div>
                    </div>
                    ...
                    {flowMode === 'A' && (
                        <div className="space-y-1.5 animate-in fade-in slide-in-from-top-2 duration-200">
                            <Label htmlFor="invite-name" className="text-xs font-semibold text-muted-foreground dark:text-muted-foreground">Họ và tên nhân viên *</Label>
                            <Input
                                id="invite-name"
                                placeholder="Nguyễn Văn A"
                                value={name}
                                onChange={(e) => setName(e.target.value)}
                                className="h-9 text-xs rounded-md border-border dark:border-slate-750 bg-white dark:bg-slate-900 focus-visible:ring-1 focus-visible:ring-blue-550"
                            />
                        </div>
                    )}
                    ...
                    {/* Role Select */}
                    <div className="space-y-1.5">
                        <Label htmlFor="invite-role" className="text-xs font-semibold text-muted-foreground dark:text-muted-foreground">Vai trò Hệ thống</Label>
                        <Select value={role} onValueChange={setRole}>
                            <SelectTrigger className="h-9 text-xs rounded-md border-border dark:border-slate-750 bg-white dark:bg-slate-900 cursor-pointer">
                                <div className="flex items-center gap-1.5">
                                    <ShieldCheck className="w-3.5 h-3.5 text-blue-500" />
                                    <SelectValue placeholder="Chọn vai trò hệ thống" />
                                </div>
                            </SelectTrigger>
                            <SelectContent className="text-xs bg-white dark:bg-slate-950 border border-border dark:border-border">
                                {roles.map((r) => (
                    ...
                    <div className="space-y-1.5">
                        <Label htmlFor="invite-department" className="text-xs font-semibold text-muted-foreground dark:text-muted-foreground">Phòng ban liên kết</Label>
                        <Select value={selectedDepartmentId} onValueChange={(val) => {
                            ...
                            <SelectContent className="text-xs bg-white dark:bg-slate-950 border border-border dark:border-border">
                                <SelectItem value="none" className="cursor-pointer text-xs">
                                    <span className="text-muted-foreground italic">Không phân phòng ban</span>
                                </SelectItem>
                                {departments.map((d) => (
                    ...
                    {selectedDepartmentId !== 'none' && (
                        <div className="space-y-1.5 animate-in fade-in slide-in-from-top-2 duration-200">
                            <Label htmlFor="invite-dept-role" className="text-xs font-semibold text-muted-foreground dark:text-muted-foreground">
                            ...
                            >
                                <SelectTrigger className="h-9 text-xs rounded-md border-border dark:border-slate-750 bg-white dark:bg-slate-900 cursor-pointer">
                                    {isFetchingDept
                                        ? <span className="flex items-center gap-1.5 text-muted-foreground"><Loader2 className="w-3 h-3 animate-spin" /> Đang kiểm tra...</span>
                                ...
                                <SelectContent className="text-xs bg-white dark:bg-slate-950 border border-border dark:border-border">
                                    ...
                                    <SelectItem value="MANAGER" className="cursor-pointer text-xs">
                                        <span className="font-semibold text-muted-foreground dark:text-muted-foreground">Phó phòng (MANAGER)</span>
                                    </SelectItem>
                                    ...
                                    <SelectItem value="GUEST" className="cursor-pointer text-xs">
                                        <span className="font-semibold text-muted-foreground dark:text-muted-foreground">Khách (GUEST)</span>
                                    </SelectItem>
                                </SelectContent>
                            </Select>
                        </div>
                    ...
                    {flowMode === 'C' && selectedDepartmentId === 'none' && (
                        <div className="space-y-1.5 animate-in fade-in duration-200">
                            <Label htmlFor="invite-message" className="text-xs font-semibold text-muted-foreground dark:text-muted-foreground">Lời nhắn trong email (Tùy chọn)</Label>
                            <Textarea
                                id="invite-message"
                                ...
                                rows={2.5}
                                className="text-xs rounded-md border-border dark:border-slate-750 bg-white dark:bg-slate-900 resize-none focus-visible:ring-1 focus-visible:ring-blue-550"
                            />
                        </div>
                    )}
                    ...
                    {/* Error display */}
                    {error && (
                        <div className="flex items-center gap-2 text-[10px] font-semibold text-red-600 bg-red-500/10 border border-red-500/20 px-3.5 py-2.5 rounded-md animate-in fade-in duration-200">
                    ...
                    {/* Footer buttons */}
                    <DialogFooter className="pt-2 border-t border-border dark:border-slate-850 flex items-center justify-end gap-2.5">
                        <Button
                            type="button"
                            variant="outline"
                            onClick={handleClose}
                            className="h-8.5 text-xs rounded-md border-border dark:border-slate-750 text-muted-foreground dark:text-muted-foreground hover:bg-muted dark:hover:bg-slate-850 cursor-pointer"
                        >
                            Hủy bỏ
                        </Button>
                        <Button
                            type="submit"
                            disabled={isLoading}
                            className={`h-8.5 text-xs font-bold rounded-md text-white transition-all cursor-pointer min-w-[130px] shadow-sm ${
                                flowMode === 'A'
