    ...
    >
      <div className="flex flex-col gap-6">
        ...
        <div className="relative group">
          ...
          <div className="relative flex items-center bg-muted border border-border/50 rounded-sm overflow-hidden focus-within:border-[#ccff00]/50 transition-colors">
            ...
            <input 
              ...
              className="w-full bg-transparent border-none text-sm text-foreground focus:outline-none focus:ring-0 placeholder-zinc-600 py-3"
            />
            {searchQuery && (
        ...
        <div className="flex flex-wrap gap-2 mt-2">
          {prompts.map((prompt, idx) => (
            <button 
              ...
              className="text-xs px-3 py-1.5 border border-border bg-muted/50 hover:bg-muted hover:border-zinc-600 text-muted-foreground rounded-sm transition-all flex items-center gap-2 group/prompt"
            >
              <MessageSquare className="w-3 h-3 text-muted-foreground group-hover/prompt:text-[#ccff00]" />
              {prompt}
            </button>
          ))}
        </div>
        ...
        <div className="mt-4 border-l-2 border-[#ccff00] pl-4 py-1">
          ...
          <p className="text-sm text-muted-foreground mb-3 italic">"{briefData?.summary || 'Tóm tắt các sự kiện nổi bật trong ngày.'}"</p>
          <div className="space-y-3">
            {briefData?.urgentMentions?.map(mention => (
              <div key={mention.id} className="text-sm text-muted-foreground">
