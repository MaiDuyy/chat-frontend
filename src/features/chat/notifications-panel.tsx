...
export default function NotificationsPanel() {
    ...
    return (
        <div className="flex flex-col h-full bg-background">
                    ...
                    {allNotifications.length > 0 && (
                        <DropdownMenu>
                            <DropdownMenuTrigger asChild>
                                <button className="ml-2 h-6 w-6 flex items-center justify-center rounded-md hover:bg-muted dark:hover:bg-slate-800 transition-colors cursor-pointer">
            ...
            {/* ── Category Tabs ───────────────────────────────────────────── */}
            <div className="flex items-center gap-0.5 px-3 py-1.5 border-b border-border dark:border-white/[0.04] shrink-0 overflow-x-auto no-scrollbar">
                        ...
                        <button
                            ...
                            className={`
                                ...
                                ${activeCategory === tab.id
                                    ...
                                    : "text-muted-foreground dark:text-muted-foreground hover:bg-muted dark:hover:bg-white/[0.05]"
                                }
                            `}
                        >
                            {tab.label}
            ...
            <div className="flex-1 overflow-y-auto no-scrollbar">
                {isLoading ? (
                    ...
                    <div className="flex flex-col items-center justify-center h-48 text-muted-foreground gap-2">
                        <Bell className="h-10 w-10 text-foreground dark:text-muted-foreground" />
                        <p className="text-xs">
                            {activeCategory === "all"
                                ? "Chưa có thông báo"
                        ...
                        {filtered.map((notification) => (
                            <div
                                ...
                                className={`
                                    ...
                                    hover:bg-muted dark:hover:bg-white/[0.03]
                                    ${!notification.isRead ? "bg-blue-50/40 dark:bg-blue-950/10" : ""}
                                `}
                            >
                                {/* Icon */}
                                <div className="flex-shrink-0 w-8 h-8 rounded-md bg-muted dark:bg-slate-800 flex items-center justify-center">
                                ...
                                <div className="flex-1 min-w-0">
                                    <div className="flex items-start justify-between gap-2 mb-0.5">
                                        ...
                                        <div className="flex items-center gap-1 shrink-0">
                                            {notification.data?.workspaceName && (
                                                <span className="text-[9px] font-bold uppercase tracking-tight text-blue-600 bg-blue-50 dark:bg-blue-950 px-1.5 py-0.5 rounded-sm">
                                                    {notification.data.workspaceName}
                                                </span>
                                            )}
                                            {/* Category badge */}
                                    ...
                                    <p className="text-[11px] text-muted-foreground dark:text-muted-foreground line-clamp-2">
                                        {notification.body}
                                    </p>
                                    {notification.data?.action && (
                                        <button
                                            ...
                                            className="mt-1.5 text-[11px] font-semibold text-blue-600 hover:text-blue-700 bg-blue-50 dark:bg-blue-950/50 px-2 py-0.5 rounded-md transition-colors cursor-pointer"
                                        >
                                ...
                                <div className="flex flex-col gap-1 shrink-0">
                                    {!notification.isRead && (
                                        <button
                                            className="h-6 w-6 flex items-center justify-center rounded-md hover:bg-muted dark:hover:bg-slate-800 transition-colors cursor-pointer"
                                            onClick={(e) => {
                                                e.stopPropagation();
                                    ...
                                    <button
                                        className="h-6 w-6 flex items-center justify-center rounded-md hover:bg-red-50 dark:hover:bg-red-950/30 transition-colors cursor-pointer group"
            ...
            {/* ── Footer summary ─────────────────────────────────────────── */}
            {filtered.length > 0 && (
                <div className="px-4 py-2 border-t border-border dark:border-white/[0.04] shrink-0 flex items-center justify-between">
...
function CategoryBadge({ type }: { type: NotificationType }) {
    ...
    return (
        <span className={`text-[9px] font-bold uppercase tracking-tight px-1.5 py-0.5 rounded-sm ${className}`}>
            {label}
        </span>
    );
}
